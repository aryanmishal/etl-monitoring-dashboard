...
test.describe('Vitals Status Page Tests', () => {
  ...
  test('Vitals Status page loads and displays table with correct columns', async ({ page }) => {
    ...
    await expect(page.getByText('HEART_RATE', { exact: true })).toBeVisible();
    await expect(page.getByText('HEART_RATE_VARIABILITY_SDNN', { exact: true })).toBeVisible();
    await expect(page.getByText('BLOOD_OXYGEN', { exact: true })).toBeVisible();
    await expect(page.getByText('RESPIRATORY_RATE', { exact: true })).toBeVisible();
  });
  ...
  test('Date filter works correctly', async ({ page }) => {
    ...
    const networkRequests = await page.evaluate(() => {
      return window.performance.getEntriesByType('resource')
        .filter(r => r.name.includes('/user-vitals'))
        .map(r => r.name);
    });
